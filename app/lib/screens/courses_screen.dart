...
class _CoursesState extends State<Courses> {
  ...
  void getData() async {
    ...
    this.favoritesCourses = await FirebaseFirestore.instance
  ...
  Widget build(BuildContext context) {
    if (!busy) {
      ...
      this.courses!.docs.forEach((courseDoc) {
        listViewChildren.add(customiseCourseTile(context, courseDoc.id,
            courseDoc.get('name'), courseDoc.get('description')));
  ...
  Widget customiseCourseTile(BuildContext context, String courseId,
    ...
    return ListTile(
      trailing: TextButton(
        onPressed: () {
          print(
              '[SUBSCRIBE BUTTON PRESSED, courseId: $courseId, user.uid: ${FirebaseAuth.instance.currentUser!.uid}]');
          ...
          FirebaseFirestore.instance
            ...
            // else if the subject is on user's favorite list, remote it
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                    content:
                        Text('Successfully removed subject from favorites')),
              );
              // refresh the page by getting courses and user favorite courses
              this.getData();
            }
